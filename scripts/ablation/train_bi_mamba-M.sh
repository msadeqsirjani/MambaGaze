#!/usr/bin/env bash
# Usage: train_bi_mamba-M.sh [clare|cldrive] [loso|kfold] [DATA_DIR] [OUT_DIR]
# Input-stream ablation: bi_mamba fed M_grud observation masks only,
...
MODEL=bi_mamba
MODEL_ARGS="--inputs m --no-optimize_threshold --no-auto_flip --no_pos_weight"
OUT_SUFFIX="-M"
